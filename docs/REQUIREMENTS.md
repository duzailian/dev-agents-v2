...
| 类别 | 编号范围 | 说明 |
|------|----------|------|
| 功能需求 | FR-01 ~ FR-25 | 修改→测试→结果→决策闭环与平台能力 |
...
#### FR-25 自动化闭环自愈 (Automated Closed-Loop Self-Healing)
- 描述：系统必须实现从“代码修改”到“启动测试”，再根据“测试结果”自动决定是否需要“继续修改代码”的完整闭环流程。
- 价值：实现真正的无人值守自动化修复，验证系统在多轮迭代中的自我修正能力。
- 验收标准：
  - 演示完整流程：初始失败 -> 自动修复 -> 测试失败/回归 -> 再次自动修复 -> 测试通过。
  - 决策引擎能准确识别“部分修复”、“引入新错误”或“完全修复”，并据此触发下一轮修改或结束任务。
  - 支持配置“最大尝试次数”防止无限循环。
- 优先级：P0
- 依赖：FR-02, FR-09, FR-17
...
#### TR-04 QEMU启动参数模板化
- 描述：系统应支持参数化的QEMU启动模板，支持不同固件类型和测试场景。
- 验收标准：
  - QEMU命令行参数可配置（CPU/内存/设备/镜像/网络）。
  - 支持多种机器类型（virt、q35等）。
  - 支持串口/监控端口配置。
- 优先级：P0

#### TR-05 QEMU串口捕获与解析
- 描述：系统应能捕获QEMU串口输出，并按时间戳存档，支持实时监控和离线分析。
- 验收标准：
  - 支持Unix socket或pty方式连接串口。
  - 串口输出实时写入日志文件。
  - 支持按关键字过滤和高亮显示。
- 优先级：P0

#### TR-06 QEMU快照与恢复
- 描述：系统应支持QEMU状态快照，用于快速恢复到已知状态。
- 验收标准：
  - 支持创建和恢复QEMU快照。
  - 快照操作耗时小于10秒。
  - 快照文件可持久化存储。
...
#### TR-07 QEMU网络配置
- 描述：系统应支持QEMU网络配置，包括用户模式网络、桥接网络等。
- 验收标准：
  - 支持用户模式网络（-netdev user）。
  - 支持端口转发配置。
  - 支持与主机网络隔离。
- 优先级：P1
...
#### TR-08 目标板连接管理
- 描述：系统应支持多种目标板连接方式（SSH、串口、BMC/IPMI）。
- 验收标准：
  - 支持SSH密钥认证和密码认证。
  - 支持串口（/dev/ttyUSB）连接。
  - 支持IPMI/BMC远程控制。
...
#### TR-10 固件刷写支持
- 描述：系统应支持向目标板刷写固件（SPI、eMMC、网络引导等）。
- 验收标准：
  - 支持通过BMC刷写BIOS/BMC固件。
  - 支持通过SSH/SCP部署文件。
  - 刷写操作具有完整性校验。
...
## 11. 知识库需求（Knowledge Base Requirements）

### 11.1 知识单元管理

#### KR-01 知识单元数据模型
- 描述：系统应定义统一的知识单元（KnowledgeUnit）数据模型，支持多种类型的知识存储。
- 验收标准：
  - KnowledgeUnit包含：ID、标题、内容、摘要、类型、标签、产品线、向量嵌入。
  - 支持知识类型：bug_fix、feature、optimization、documentation。
  - 支持元数据扩展字段。
- 优先级：P0
- 参考：KNOWLEDGE_SCHEMA.md

#### KR-02 知识自动沉淀
- 描述：系统应在每次迭代结束后，自动将关键信息结构化写入知识库。
- 验收标准：
  - 自动记录：输入diff、环境配置、测试计划、结果摘要、结论和采取动作。
  - 支持按产品线标签检索。
  - 支持手动触发知识沉淀。
- 优先级：P0

#### KR-03 知识版本管理
- 描述：系统应支持知识单元的版本管理，记录变更历史。
- 验收标准：
  - 每次更新生成新版本。
  - 支持查看历史版本。
  - 支持回滚到指定版本。
...
#### KR-04 语义检索能力
- 描述：系统应支持基于向量相似度的语义检索。
- 验收标准：
  - 使用向量数据库（Qdrant）存储知识嵌入。
  - 检索响应时间<2秒。
  - 返回结果包含相似度分数。
- 优先级：P0

#### KR-05 混合检索支持
- 描述：系统应支持向量检索与关键词检索的混合模式。
- 验收标准：
  - 支持纯向量检索、纯关键词检索、混合检索。
  - 混合检索支持权重配置。
  - 支持按标签/产品线过滤。
...
#### KR-08 文档导入解析
- 描述：系统应支持从外部文档导入知识，包括Markdown、TXT、Word、PDF、Excel格式。
- 验收标准：
  - Phase 2必须支持：Markdown (.md)、纯文本 (.txt)。
  - Phase 3必须支持：Word (.docx)、PDF (.pdf)、Excel (.xlsx)。
  - 导入过程记录来源、版本、时间、标签。
- 优先级：P1
- 参考：FR-22

#### KR-09 产品线标签体系
- 描述：系统应支持完整的产品线标签体系，用于知识分类和过滤。
- 验收标准：
  - 支持多维度标签：SoC类型、固件栈、板型、功能模块。
  - 标签可层级组织（如 BMC > X100 > Memory）。
  - 支持按标签统计知识分布。
- 优先级：P0
...
**文档版本**：v2.0
**更新日期**：2026-01-30
**状态**：已补充TR-05~TR-14和KR-01~KR-18章节
